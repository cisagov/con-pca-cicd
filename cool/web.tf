...
locals {
  ...
  web_environment = {
    "API_URL" : "https://${aws_route53_record.sharedservices_internal_api.name}"
    "API_URL_HEADLESS" : "https://${aws_route53_record.sharedservices_internal_api.name}"
    ...
    "OAUTH_REDIRECT_URL" : "https://${aws_route53_record.sharedservices_internal_web.name}"
...
resource "aws_lb_listener" "web" {
  load_balancer_arn = module.web_alb.alb_arn
  port              = local.web_load_balancer_port
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-2016-08"
  certificate_arn   = module.web_certs.this_acm_certificate_arn
