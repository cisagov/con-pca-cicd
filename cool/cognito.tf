...
resource "aws_cognito_user_pool_client" "client" {
  ...
  callback_urls                        = ["https://${module.internal_alb.alb_dns_name}", "https://${aws_route53_record.internal.name}"]
  ...
  logout_urls                          = ["https://${module.internal_alb.alb_dns_name}", "https://${aws_route53_record.internal.name}"]
